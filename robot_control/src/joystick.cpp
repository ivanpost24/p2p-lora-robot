...
JoyState joystick_read() {
    JoyState s;
    s.x      = _read_axis(PIN_VRX);
    s.y      = _read_axis(PIN_VRY);
    s.button = (digitalRead(PIN_SW) == LOW);
    return s;
}
...
void joystick_debug_print() {
    ...
    int proc_x = _read_axis(PIN_VRX);
    int proc_y = _read_axis(PIN_VRY);
    ...
    display_print_line(5, "Proc X: ", proc_x);
    display_print_line(6, "Proc Y: ", proc_y);
